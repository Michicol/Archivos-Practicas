"""
Codigo principal para descargar datos
(Actual hasta el 26 de enero de 2020)
"""

from datetime import datetime

#Requests - estandar o anonimo
import requests
...
def lenght(data):
	""" 
	Para obtener la longitud/extension de los datos bajados
	data=list(dict())
	"""
	if len(data)!=0:
		...
		points=len(data)
		return 'Datos desde {0} hasta {1}, con un total de {2} puntos.\n'.format(first_date,last_date,points)
    else:
        return 'Esta cosa está vacía :c.\n'

def query(company,anonymous=True):
    """
    type(company)=str
    Funcion Yahoo Finance Query
    Se obtiene una lista de diccionarios con las siguientes claves:
    ...
    if anonymous:
        #req=TorRequest(proxy_port=9050, ctrl_port=9051, password=None)
        req=TorRequest()
...
def get(yahoo_code,inicio,fin):
    """
    bajar datos historicos de yahoo finance
